            ...
            <div class="cards">
                <div class="card">
                    <form class="w-full" action="{{ route('utility.lipa-na-mpesa') }}" method="POST">
                        @csrf
                        <div class="status flex flex-col sm:flex-row">
                            <label class="sm:mr-4 text-lg font-semibold">Amount</label>
                            <input type="text" class="w-full" placeholder="Enter Amount" name="amount" required/>
                        </div>
